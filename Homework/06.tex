...
\begin{enumerate}
  ...
  \item % #4
    ...
    \begin{proof}
      ...
      Let
      \[
        m = \sum{e \in \delta^+(X)}{}{u(e)}
      \]
      Consider the following sets of edges.
      \begin{align*}
        A &= \set{(v, w) : v \in X, v \not\in Y, w \not\in X, w \not\in Y} \\
        B &= \set{(v, w) : v \in X, v \in Y, w \in X, w \not\in Y} \\
        C &= \set{(v, w) : v \in X, v \in Y, w \not\in X, w \in Y} \\
        D &= \set{(v, w) : v \not\in X, v \in Y, w \not\in X, w \not\in Y} \\
        E &= \set{(v, w) : v \in X, v \in Y, w \not\in X, w \not\in Y} \\
      \end{align*}
      The sets $\delta^+(X)$ and $\delta^+(Y)$ can be partitioned in terms of
      these sets.
      Notice $\delta^+(X) = A \cup C \cup E$, because edges from $X$ to $X^c$ may
      also go from $Y$ to $Y$, $Y^c
    \end{proof}
